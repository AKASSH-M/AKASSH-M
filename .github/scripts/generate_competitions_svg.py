...
OUT  = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "..", "assets", "competitions.svg")
...
# Data
JOBS = [
    ("H@CIT Cod-a-thon'26", "", "", PURPLE, [
        "Winner",
    ]),
    ("Statathon'26 — MOSPI", "", "", GREEN, [
        "Runner-Up",
    ]),
    ("MumbaiHacks — HCL", "", "", CYAN, [
        "Finalist",
    ]),
    ("Handloom Hackathon — IIT Delhi", "", "", YELLOW, [
        "Finalist",
    ])
]
...
svg.append(f'<text x="{W//2}" y="{TITLEBAR//2+5}" fill="{TITLE_CLR}" font-size="14" '
           f'text-anchor="middle">akassh@github: ~/competitions</text>')
...
for company, role, period, color, bullets in JOBS:
    ...
    header = (f'<text xml:space="preserve" x="{PAD}" y="{y + LINE_H * 0.75:.0f}" font-size="{FONT+2}">'
              f'<tspan fill="{DIM}">&gt; </tspan>'
              f'<tspan fill="{color}" font-weight="bold">{html.escape(company)}</tspan></text>')
    ...
    svg.append(f'<rect y="{y+2}" width="10" height="{LINE_H-4}" fill="{CURSOR}" opacity="0">'
...
svg.append(f'<rect x="{PAD}" y="{y - LINE_H + 4}" width="10" height="{LINE_H-8}" fill="{CURSOR}">'
